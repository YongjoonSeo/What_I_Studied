...
while T:
    ...
    for dust in dusts:
        ...
        if y != 0 and y != R-1 and y != up[0] and y != down[0] and x != 0 and x != C-1:
            notinrange.add((y,x))
        if y+1 < R and field[y+1][x] != -1:
            if field[y+1][x] == 0 and y+1 != R-1 and y+1 != up[0] and y+1 != down[0] and x != 0 and x != C-1:
                notinrange.add((y+1, x))
            field[y+1][x] += A//5
            cnt += 1
        if y-1 >= 0 and field[y-1][x] != -1:
            if field[y-1][x] == 0 and y-1 != 0 and y-1 != up[0] and y-1 != down[0] and x != 0 and x != C-1:
                notinrange.add((y-1, x))
            field[y-1][x] += A//5
            cnt += 1
        if x+1 < C and field[y][x+1] != -1:
            if field[y][x+1] == 0 and y != R-1 and y != up[0] and y != down[0] and x+1 != C-1:
                notinrange.add((y, x+1))
            field[y][x+1] += A//5
            cnt += 1
        if x-1 >= 0 and field[y][x-1] != -1:
            if field[y][x-1] == 0 and y != R-1 and y != up[0] and y != down[0] and x-1 != 0:
                notinrange.add((y, x-1))
            field[y][x-1] += A//5
            cnt += 1
        field[y][x] -= (A//5) * cnt
