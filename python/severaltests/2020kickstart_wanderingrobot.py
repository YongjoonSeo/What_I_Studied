# W columns (1 to W) H rows (1 to H)
# (x, y) - xth column & yth row <<<------
# begins from (1, 1)
# from topleft (L,U) to bottomright (R,D) have been removed
# to bottom right direction with equal probability
# print probability that she passes the challenge
...
for t in range(1, T+1):
    ...
    field = [[1 if (j == 0 or i == 0) else 0 for i in range(W)] for j in range(H)]

    for i in range(1, H):
        for j in range(1, W):
            field[i][j] = field[i-1][j] + field[i][j-1]
    result = field[D-1][R-1] / field[H-1][W-1]
