...
class linkedlist:
    ...
    def extend(self, llist, idx=None):
        inserthead = llist.head
        inserttail = llist.get(-1)
        insertlength = llist.length
        if idx == None:
            idx = self.length
        ...
        if idx == 0 or -idx == self.length:
            temphead = self.head
            self.head = inserthead
            inserttail.next = temphead
        else:
            prevhead = self.get(idx-1)
            targethead = prevhead.next
            prevhead.next = inserthead
            inserttail.next = targethead
        self.length += insertlength
